...
var goodUIntConversions = map[interface{}]uint64{
	float32(32.0):         32,
...
var badUIntConversions = []interface{}{
	errors.New("some error"),
	"some random string",
}

func TestShouldAppendGoodConversionsToUIntCol(t *testing.T) {
	var err error
	var at interface{}
	var ok bool
...
func TestShouldNotAppendBadConversionsToUIntCol(t *testing.T) {
	var err error

	c := new(UIntCol)
	for _, input := range badUIntConversions {
		err = c.Append(input)
...
func TestShouldGetGoStringFromUIntCol(t *testing.T) {
	c := new(UIntCol)
	val := 623
	c.Append(val)
	c.Append(val)
	expected := fmt.Sprintf("[]uint64{%#x, %#x}", val, val)
	value := c.String()
	if value != expected {
...
func TestShouldReturnNilFalseIfIndexDoesNotExistInUIntCol(t *testing.T) {
	c := new(UIntCol)
	v, ok := c.At(10)
	assert.Nil(t, v)
	assert.False(t, ok)
